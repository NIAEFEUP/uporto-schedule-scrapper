    ...
    def parse_course_plan(self, response):
        ...
        for group_div in group_divs:
            ...
            if group_title:
                ...
                # Generate a numeric course group ID by combining course_id and a hash of the group name
                # We concatenate course_id with the hash of the group name, then convert it to an integer
                hash_object = hashlib.md5(group_name.encode('utf-8'))
                hashed_group_name = hash_object.hexdigest()
                numeric_group_id = int(hashed_group_name, 16) % 10**8  # Limit to a certain number of digits
                ...
                yield course_group_item  # Yielding CourseGroup item for later processing

                # Process course units in the group
